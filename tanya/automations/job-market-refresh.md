...
2. Search current public vacancies posted since the previous scheduled run. Search every board in `requiredJobBoards` separately on every run: SEEK New Zealand (`nz.seek.com`), SEEK Australia (`seek.com.au`), and Trade Me Jobs (`trademe.co.nz`, Jobs section). Use domain-targeted web searches across the target role families; searching one SEEK market does not cover both. If direct access is unavailable, search publicly indexed listings and verify details against employer career pages where possible. Never invent vacancies to fill a source gap. Also search employer career pages, LinkedIn, and reputable NZ recruiters. Roles must explicitly support Auckland hybrid, North Shore, remote New Zealand, or NZ-based work for Australian organisations; Australia-only remote roles do not qualify. Deduplicate vacancies found on multiple boards.
